...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table('tools_database', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('label', self.gf('django.db.models.fields.CharField')(unique=True, max_length=128)),
        ))
        db.send_create_signal('tools', ['Database'])

        # Adding M2M table for field database on 'ToolsProfile'
        db.create_table('tools_toolsprofile_database', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('toolsprofile', models.ForeignKey(orm['tools.toolsprofile'], null=False)),
            ('database', models.ForeignKey(orm['tools.database'], null=False))
        ))
        db.create_unique('tools_toolsprofile_database', ['toolsprofile_id', 'database_id'])


    def backwards(self, orm):
        # Deleting model 'Database'
        db.delete_table('tools_database')

        # Removing M2M table for field database on 'ToolsProfile'
        db.delete_table('tools_toolsprofile_database')
